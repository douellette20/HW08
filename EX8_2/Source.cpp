...
bool PlaceQueen(int column)
{
	...
	else
	{
		...
		for (int row = 0; row < RowPlacement.size(); ++row)
		{
			...
			// DONE: Check if this row has already been assigned
			...
			// DONE: Assign column to Row[row]
			RowPlacement[row] = column;
			

			// DONE: Recursively call PlaceQueen to place a queen in the next column.
			...
			// DONE: Unassign the assignment of the column's queen to this row, and try the next one
