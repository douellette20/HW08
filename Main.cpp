...
int binary_search(const vector<T> &items, int first, int last, T target) {
	// DONE: Implement recursive binary search
	if (first > last)
		return -1;
	else
	{
		...
		if (items[midIndex] == target)
			return midIndex;
		else if (target < items[midIndex])
			return binary_search(items, first, midIndex - 1, target);
		else
			return binary_search(items, midIndex + 1, last, target);
	}
}
